...
for power, battery, and IR transmit see [07_power_and_ir.md](07_power_and_ir.md).
...
One push button (Send) and one optional toggle (Swing), direct to GPIO.
...
ATmega328P
  ...
  PD2 PD3 PD4  ◄──  Temp       (SR16, 8 pos, 3 code lines)
  PD5          ◄──  Send button        (pull-up, active-low)
  PD6          ◄──  Swing toggle (opt) (pull-up, active-low)
  PD7 (OC2A)  ──►  IR LED driver
...
The 1 MΩ value is a first-order budget term on the target MCU (nRF52840 ~2 µA floor).
Bench-validate that reads are still reliable at 1 MΩ before treating it as settled.
See [07_power_and_ir.md §3](07_power_and_ir.md) for the full leakage budget.

---

## 4. Buttons

**Send** (PD5) — push button, triggers IR transmission.
**Swing** (PD6) — on/off toggle, optional.

Each connects between a GPIO pin and GND. The pin uses the MCU internal pull-up
(`INPUT_PULLUP`) — pressed/on = LOW, released/off = HIGH.

```
GPIO (INPUT_PULLUP) ──┬── button/toggle ── GND
                      └── (reads HIGH at rest, LOW when pressed)
```

No external resistors needed. Each line is armed for both-edge interrupt for wake.
...
sleep  (SLEEP_MODE_PWR_DOWN, all lines PCINT-armed)
  │
  │  any code-line edge  ──or──  Send / Swing edge
  ▼
wake
...
The device only transmits on explicit Send press — knob moves alone do not trigger IR.
This avoids spurious sends while adjusting settings.
