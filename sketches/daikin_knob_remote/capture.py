...
# ///
"""Capture serial output from the daikin_knob_remote sketch.
...
Usage:
    uv run sketches/daikin_knob_remote/capture.py /dev/cu.usbserial-10
    uv run sketches/daikin_knob_remote/capture.py /dev/cu.usbserial-10 -o dump.txt
    uv run sketches/daikin_knob_remote/capture.py /dev/cu.usbserial-10 --sends-only
"""
...
def main() -> None:
    ...
    p.add_argument("-s", "--sends-only", action="store_true",
                   help="only print SEND/SENT transmit lines (filter out knob readouts)")
    args = p.parse_args()
    ...
    with serial.Serial(args.port, args.baud, timeout=1) as s:
        ...
        try:
            while True:
                ...
                if out:
                    out.write(line)
                if args.sends_only and not (line.startswith("SEND") or line.startswith("SENT")):
                    continue
